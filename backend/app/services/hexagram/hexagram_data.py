"""六十四卦完整数据

数据来源：周易原文（King Wen 序列）
每个卦包含：卦号、卦名、Unicode符号、上下卦、卦象含义、彖辞、象辞、六爻爻辞。

爻的二进制映射（阳=1, 阴=0, 从下到上）：
    乾(☰)=111, 兑(☱)=110, 离(☲)=101, 震(☳)=100
    巽(☴)=011, 坎(☵)=010, 艮(☶)=001, 坤(☷)=000

卦号通过 (上卦, 下卦) 查表获得，遵循 King Wen 传统序列。
"""

from typing import Optional


# 八卦基础数据：名称 -> (二进制值, Unicode符号, 自然象征)
TRIGRAMS = {
    "乾": (0b111, "☰", "天"),
    "兑": (0b110, "☱", "泽"),
    "离": (0b101, "☲", "火"),
    "震": (0b100, "☳", "雷"),
    "巽": (0b011, "☴", "风"),
    "坎": (0b010, "☵", "水"),
    "艮": (0b001, "☶", "山"),
    "坤": (0b000, "☷", "地"),
}

# 二进制值 -> 卦名 的反向映射
_BINARY_TO_TRIGRAM = {v[0]: name for name, v in TRIGRAMS.items()}

# King Wen 序列：(上卦二进制, 下卦二进制) -> 卦号
# 这是传统的六十四卦排序，不是简单的二进制编码
_KING_WEN_TABLE: dict[tuple[int, int], int] = {
    (0b111, 0b111): 1,   # 乾为天
    (0b000, 0b000): 2,   # 坤为地
    (0b010, 0b100): 3,   # 水雷屯
    (0b001, 0b010): 4,   # 山水蒙
    (0b010, 0b111): 5,   # 水天需
    (0b111, 0b010): 6,   # 天水讼
    (0b000, 0b010): 7,   # 地水师
    (0b010, 0b000): 8,   # 水地比
    (0b011, 0b111): 9,   # 风天小畜
    (0b111, 0b011): 10,  # 天泽履
    (0b000, 0b111): 11,  # 地天泰
    (0b111, 0b000): 12,  # 天地否
    (0b111, 0b101): 13,  # 天火同人
    (0b101, 0b111): 14,  # 火天大有
    (0b000, 0b001): 15,  # 地山谦
    (0b100, 0b000): 16,  # 雷地豫
    (0b110, 0b100): 17,  # 泽雷随
    (0b001, 0b011): 18,  # 山风蛊
    (0b000, 0b110): 19,  # 地泽临
    (0b011, 0b000): 20,  # 风地观
    (0b101, 0b100): 21,  # 火雷噬嗑
    (0b001, 0b101): 22,  # 山火贲
    (0b001, 0b000): 23,  # 山地剥
    (0b000, 0b100): 24,  # 地雷复
    (0b111, 0b100): 25,  # 天雷无妄
    (0b001, 0b111): 26,  # 山天大畜
    (0b001, 0b100): 27,  # 山雷颐
    (0b110, 0b011): 28,  # 泽风大过
    (0b010, 0b010): 29,  # 坎为水
    (0b101, 0b101): 30,  # 离为火
    (0b110, 0b001): 31,  # 泽山咸
    (0b100, 0b011): 32,  # 雷风恒
    (0b111, 0b001): 33,  # 天山遁
    (0b100, 0b111): 34,  # 雷天大壮
    (0b101, 0b000): 35,  # 火地晋
    (0b000, 0b101): 36,  # 地火明夷
    (0b011, 0b101): 37,  # 风火家人
    (0b101, 0b110): 38,  # 火泽睽
    (0b010, 0b001): 39,  # 水山蹇
    (0b100, 0b010): 40,  # 雷水解
    (0b001, 0b110): 41,  # 山泽损
    (0b011, 0b100): 42,  # 风雷益
    (0b110, 0b111): 43,  # 泽天夬
    (0b111, 0b110): 44,  # 天风姤
    (0b110, 0b000): 45,  # 泽地萃
    (0b000, 0b011): 46,  # 地风升
    (0b110, 0b010): 47,  # 泽水困
    (0b010, 0b011): 48,  # 水风井
    (0b110, 0b101): 49,  # 泽火革
    (0b101, 0b011): 50,  # 火风鼎
    (0b100, 0b100): 51,  # 震为雷
    (0b001, 0b001): 52,  # 艮为山
    (0b011, 0b001): 53,  # 风山渐
    (0b100, 0b110): 54,  # 雷泽归妹
    (0b100, 0b101): 55,  # 雷火丰
    (0b101, 0b001): 56,  # 火山旅
    (0b011, 0b011): 57,  # 巽为风
    (0b110, 0b110): 58,  # 兑为泽
    (0b011, 0b010): 59,  # 风水涣
    (0b010, 0b110): 60,  # 水泽节
    (0b011, 0b110): 61,  # 风泽中孚
    (0b100, 0b001): 62,  # 雷山小过
    (0b010, 0b101): 63,  # 水火既济
    (0b101, 0b010): 64,  # 火水未济
}

# 反向映射：卦号 -> (上卦二进制, 下卦二进制)
_NUM_TO_TRIGRAM_PAIR: dict[int, tuple[int, int]] = {v: k for k, v in _KING_WEN_TABLE.items()}


def _hexagram_symbol(upper_bin: int, lower_bin: int) -> str:
    """根据上下卦二进制生成六爻Unicode符号（从下到上排列）。

    返回6个字符的字符串，每个字符代表一爻（阳爻⚊或阴爻⚋）。
    索引0是初爻（最下），索引5是上爻（最上）。
    """
    bits = []
    for i in range(3):
        bits.append((lower_bin >> i) & 1)
    for i in range(3):
        bits.append((upper_bin >> i) & 1)
    return "".join("⚊" if b else "⚋" for b in bits)


# 六十四卦完整数据
# 每个卦包含：num, name, symbol, trigram_upper, trigram_lower,
#              meaning, image, judgement, yao_texts(6条爻辞)
HEXAGRAMS: dict[int, dict] = {
    1: {
        "num": 1,
        "name": "乾",
        "symbol": "☰☰",
        "trigram_upper": "乾",
        "trigram_lower": "乾",
        "meaning": "天行健，君子以自强不息",
        "image": "天行健，君子以自强不息。",
        "judgement": "乾：元，亨，利，贞。",
        "yao_texts": [
            "初九：潜龙勿用。",
            "九二：见龙在田，利见大人。",
            "九三：君子终日乾乾，夕惕若厉，无咎。",
            "九四：或跃在渊，无咎。",
            "九五：飞龙在天，利见大人。",
            "上九：亢龙有悔。",
        ],
    },
    2: {
        "num": 2,
        "name": "坤",
        "symbol": "☷☷",
        "trigram_upper": "坤",
        "trigram_lower": "坤",
        "meaning": "地势坤，君子以厚德载物",
        "image": "地势坤，君子以厚德载物。",
        "judgement": "坤：元，亨，利牝马之贞。君子有攸往，先迷后得主。利西南得朋，东北丧朋。安贞吉。",
        "yao_texts": [
            "初六：履霜，坚冰至。",
            "六二：直、方、大，不习无不利。",
            "六三：含章可贞。或从王事，无成有终。",
            "六四：括囊，无咎无誉。",
            "六五：黄裳，元吉。",
            "上六：龙战于野，其血玄黄。",
        ],
    },
    3: {
        "num": 3,
        "name": "屯",
        "symbol": "☵☳",
        "trigram_upper": "坎",
        "trigram_lower": "震",
        "meaning": "云雷屯，君子以经纶",
        "image": "云雷，屯。君子以经纶。",
        "judgement": "屯：元，亨，利，贞。勿用有攸往，利建侯。",
        "yao_texts": [
            "初九：磐桓，利居贞，利建侯。",
            "六二：屯如邅如，乘马班如。匪寇婚媾，女子贞不字，十年乃字。",
            "六三：即鹿无虞，惟入于林中，君子几不如舍，往吝。",
            "六四：乘马班如，求婚媾，往吉，无不利。",
            "九五：屯其膏，小贞吉，大贞凶。",
            "上六：乘马班如，泣血涟如。",
        ],
    },
    4: {
        "num": 4,
        "name": "蒙",
        "symbol": "☶☵",
        "trigram_upper": "艮",
        "trigram_lower": "坎",
        "meaning": "山下出泉，蒙，君子以果行育德",
        "image": "山下出泉，蒙。君子以果行育德。",
        "judgement": "蒙：亨。匪我求童蒙，童蒙求我。初筮告，再三渎，渎则不告。利贞。",
        "yao_texts": [
            "初六：发蒙，利用刑人，用说桎梏，以往吝。",
            "九二：包蒙，吉。纳妇，吉。子克家。",
            "六三：勿用取女，见金夫，不有躬，无攸利。",
            "六四：困蒙，吝。",
            "六五：童蒙，吉。",
            "上九：击蒙，不利为寇，利御寇。",
        ],
    },
    5: {
        "num": 5,
        "name": "需",
        "symbol": "☵☰",
        "trigram_upper": "坎",
        "trigram_lower": "乾",
        "meaning": "云上于天，需，君子以饮食宴乐",
        "image": "云上于天，需。君子以饮食宴乐。",
        "judgement": "需：有孚，光亨，贞吉。利涉大川。",
        "yao_texts": [
            "初九：需于郊，利用恒，无咎。",
            "九二：需于沙，小有言，终吉。",
            "九三：需于泥，致寇至。",
            "六四：需于血，出自穴。",
            "九五：需于酒食，贞吉。",
            "上六：入于穴，有不速之客三人来，敬之终吉。",
        ],
    },
    6: {
        "num": 6,
        "name": "讼",
        "symbol": "☰☵",
        "trigram_upper": "乾",
        "trigram_lower": "坎",
        "meaning": "天与水违行，讼，君子以作事谋始",
        "image": "天与水违行，讼。君子以作事谋始。",
        "judgement": "讼：有孚窒惕，中吉，终凶。利见大人，不利涉大川。",
        "yao_texts": [
            "初六：不永所事，小有言，终吉。",
            "九二：不克讼，归而逋，其邑人三百户，无眚。",
            "六三：食旧德，贞厉，终吉。或从王事，无成。",
            "九四：不克讼，复即命，渝，安贞吉。",
            "九五：讼，元吉。",
            "上九：或锡之鞶带，终朝三褫之。",
        ],
    },
    7: {
        "num": 7,
        "name": "师",
        "symbol": "☷☵",
        "trigram_upper": "坤",
        "trigram_lower": "坎",
        "meaning": "地中有水，师，君子以容民畜众",
        "image": "地中有水，师。君子以容民畜众。",
        "judgement": "师：贞，丈人吉，无咎。",
        "yao_texts": [
            "初六：师出以律，否臧凶。",
            "九二：在师中，吉无咎，王三锡命。",
            "六三：师或舆尸，凶。",
            "六四：师左次，无咎。",
            "六五：田有禽，利执言，无咎。长子帅师，弟子舆尸，贞凶。",
            "上六：大君有命，开国承家，小人勿用。",
        ],
    },
    8: {
        "num": 8,
        "name": "比",
        "symbol": "☵☷",
        "trigram_upper": "坎",
        "trigram_lower": "坤",
        "meaning": "地上有水，比，先王以建万国，亲诸侯",
        "image": "地上有水，比。先王以建万国，亲诸侯。",
        "judgement": "比：吉。原筮元永贞，无咎。不宁方来，后夫凶。",
        "yao_texts": [
            "初六：有孚比之，无咎。有孚盈缶，终来有它，吉。",
            "六二：比之自内，贞吉。",
            "六三：比之匪人。",
            "六四：外比之，贞吉。",
            "九五：显比，王用三驱，失前禽，邑人不诫，吉。",
            "上六：比之无首，凶。",
        ],
    },
    9: {
        "num": 9,
        "name": "小畜",
        "symbol": "☴☰",
        "trigram_upper": "巽",
        "trigram_lower": "乾",
        "meaning": "风行天上，小畜，君子以懿文德",
        "image": "风行天上，小畜。君子以懿文德。",
        "judgement": "小畜：亨。密云不雨，自我西郊。",
        "yao_texts": [
            "初九：复自道，何其咎，吉。",
            "九二：牵复，吉。",
            "九三：舆说辐，夫妻反目。",
            "六四：有孚，血去惕出，无咎。",
            "九五：有孚挛如，富以其邻。",
            "上九：既雨既处，尚德载，妇贞厉。月几望，君子征凶。",
        ],
    },
    10: {
        "num": 10,
        "name": "履",
        "symbol": "☰☱",
        "trigram_upper": "乾",
        "trigram_lower": "兑",
        "meaning": "上天下泽，履，君子以辩上下，定民志",
        "image": "上天下泽，履。君子以辩上下，定民志。",
        "judgement": "履：履虎尾，不咥人，亨。",
        "yao_texts": [
            "初九：素履往，无咎。",
            "九二：履道坦坦，幽人贞吉。",
            "六三：眇能视，跛能履，履虎尾，咥人，凶。武人为于大君。",
            "九四：履虎尾，愬愬终吉。",
            "九五：夬履，贞厉。",
            "上九：视履考祥，其旋元吉。",
        ],
    },
    11: {
        "num": 11,
        "name": "泰",
        "symbol": "☷☰",
        "trigram_upper": "坤",
        "trigram_lower": "乾",
        "meaning": "天地交，泰，后以财成天地之道",
        "image": "天地交，泰。后以财成天地之道，辅相天地之宜，以左右民。",
        "judgement": "泰：小往大来，吉亨。",
        "yao_texts": [
            "初九：拔茅茹，以其汇，征吉。",
            "九二：包荒，用冯河，不遐遗，朋亡，得尚于中行。",
            "九三：无平不陂，无往不复，艰贞无咎。勿恤其孚，于食有福。",
            "六四：翩翩不富，以其邻，不戒以孚。",
            "六五：帝乙归妹，以祉元吉。",
            "上六：城复于隍，勿用师。自邑告命，贞吝。",
        ],
    },
    12: {
        "num": 12,
        "name": "否",
        "symbol": "☰☷",
        "trigram_upper": "乾",
        "trigram_lower": "坤",
        "meaning": "天地不交，否，君子以俭德辟难",
        "image": "天地不交，否。君子以俭德辟难，不可荣以禄。",
        "judgement": "否：否之匪人，不利君子贞，大往小来。",
        "yao_texts": [
            "初六：拔茅茹，以其汇，贞吉亨。",
            "六二：包承，小人吉，大人否亨。",
            "六三：包羞。",
            "九四：有命无咎，畴离祉。",
            "九五：休否，大人吉。其亡其亡，系于苞桑。",
            "上九：倾否，先否后喜。",
        ],
    },
    13: {
        "num": 13,
        "name": "同人",
        "symbol": "☰☲",
        "trigram_upper": "乾",
        "trigram_lower": "离",
        "meaning": "天与火，同人，君子以类族辨物",
        "image": "天与火，同人。君子以类族辨物。",
        "judgement": "同人：同人于野，亨。利涉大川，利君子贞。",
        "yao_texts": [
            "初九：同人于门，无咎。",
            "六二：同人于宗，吝。",
            "九三：伏戎于莽，升其高陵，三岁不兴。",
            "九四：乘其墉，弗克攻，吉。",
            "九五：同人，先号啕而后笑。大师克相遇。",
            "上九：同人于郊，无悔。",
        ],
    },
    14: {
        "num": 14,
        "name": "大有",
        "symbol": "☲☰",
        "trigram_upper": "离",
        "trigram_lower": "乾",
        "meaning": "火在天上，大有，君子以遏恶扬善",
        "image": "火在天上，大有。君子以遏恶扬善，顺天休命。",
        "judgement": "大有：元亨。",
        "yao_texts": [
            "初九：无交害，匪咎，艰则无咎。",
            "九二：大车以载，有攸往，无咎。",
            "九三：公用亨于天子，小人弗克。",
            "九四：匪其彭，无咎。",
            "六五：厥孚交如，威如，吉。",
            "上九：自天祐之，吉无不利。",
        ],
    },
    15: {
        "num": 15,
        "name": "谦",
        "symbol": "☷☶",
        "trigram_upper": "坤",
        "trigram_lower": "艮",
        "meaning": "地中有山，谦，君子以裒多益寡",
        "image": "地中有山，谦。君子以裒多益寡，称物平施。",
        "judgement": "谦：亨，君子有终。",
        "yao_texts": [
            "初六：谦谦君子，用涉大川，吉。",
            "六二：鸣谦，贞吉。",
            "九三：劳谦，君子有终，吉。",
            "六四：无不利，撝谦。",
            "六五：不富以其邻，利用侵伐，无不利。",
            "上六：鸣谦，利用行师，征邑国。",
        ],
    },
    16: {
        "num": 16,
        "name": "豫",
        "symbol": "☳☷",
        "trigram_upper": "震",
        "trigram_lower": "坤",
        "meaning": "雷出地奋，豫，先王以作乐崇德",
        "image": "雷出地奋，豫。先王以作乐崇德，殷荐之上帝，以配祖考。",
        "judgement": "豫：利建侯行师。",
        "yao_texts": [
            "初六：鸣豫，凶。",
            "六二：介于石，不终日，贞吉。",
            "六三：盱豫，悔。迟有悔。",
            "九四：由豫，大有得。勿疑，朋盍簪。",
            "六五：贞疾，恒不死。",
            "上六：冥豫，成有渝，无咎。",
        ],
    },
    17: {
        "num": 17,
        "name": "随",
        "symbol": "☱☳",
        "trigram_upper": "兑",
        "trigram_lower": "震",
        "meaning": "泽中有雷，随，君子以向晦入宴息",
        "image": "泽中有雷，随。君子以向晦入宴息。",
        "judgement": "随：元亨利贞，无咎。",
        "yao_texts": [
            "初九：官有渝，贞吉。出门交有功。",
            "六二：系小子，失丈夫。",
            "六三：系丈夫，失小子。随有求得，利居贞。",
            "九四：随有获，贞凶。有孚在道，以明，何咎。",
            "九五：孚于嘉，吉。",
            "上六：拘系之，乃从维之。王用亨于西山。",
        ],
    },
    18: {
        "num": 18,
        "name": "蛊",
        "symbol": "☶☴",
        "trigram_upper": "艮",
        "trigram_lower": "巽",
        "meaning": "山下有风，蛊，君子以振民育德",
        "image": "山下有风，蛊。君子以振民育德。",
        "judgement": "蛊：元亨，利涉大川。先甲三日，后甲三日。",
        "yao_texts": [
            "初六：干父之蛊，有子，考无咎，厉终吉。",
            "九二：干母之蛊，不可贞。",
            "九三：干父之蛊，小有悔，无大咎。",
            "六四：裕父之蛊，往见吝。",
            "六五：干父之蛊，用誉。",
            "上九：不事王侯，高尚其事。",
        ],
    },
    19: {
        "num": 19,
        "name": "临",
        "symbol": "☷☱",
        "trigram_upper": "坤",
        "trigram_lower": "兑",
        "meaning": "泽上有地，临，君子以教思无穷",
        "image": "泽上有地，临。君子以教思无穷，容保民无疆。",
        "judgement": "临：元亨利贞。至于八月有凶。",
        "yao_texts": [
            "初九：咸临，贞吉。",
            "九二：咸临，吉无不利。",
            "六三：甘临，无攸利。既忧之，无咎。",
            "六四：至临，无咎。",
            "六五：知临，大君之宜，吉。",
            "上六：敦临，吉无咎。",
        ],
    },
    20: {
        "num": 20,
        "name": "观",
        "symbol": "☴☷",
        "trigram_upper": "巽",
        "trigram_lower": "坤",
        "meaning": "风行地上，观，先王以省方观民设教",
        "image": "风行地上，观。先王以省方观民设教。",
        "judgement": "观：盥而不荐，有孚颙若。",
        "yao_texts": [
            "初六：童观，小人无咎，君子吝。",
            "六二：窥观，利女贞。",
            "六三：观我生，进退。",
            "六四：观国之光，利用宾于王。",
            "九五：观我生，君子无咎。",
            "上九：观其生，君子无咎。",
        ],
    },
    21: {
        "num": 21,
        "name": "噬嗑",
        "symbol": "☲☳",
        "trigram_upper": "离",
        "trigram_lower": "震",
        "meaning": "雷电，噬嗑，先王以明罚敕法",
        "image": "雷电，噬嗑。先王以明罚敕法。",
        "judgement": "噬嗑：亨。利用狱。",
        "yao_texts": [
            "初九：屦校灭趾，无咎。",
            "六二：噬肤灭鼻，无咎。",
            "六三：噬腊肉，遇毒；小吝，无咎。",
            "九四：噬干胏，得金矢，利艰贞，吉。",
            "六五：噬干肉，得黄金，贞厉，无咎。",
            "上九：何校灭耳，凶。",
        ],
    },
    22: {
        "num": 22,
        "name": "贲",
        "symbol": "☶☲",
        "trigram_upper": "艮",
        "trigram_lower": "离",
        "meaning": "山下有火，贲，君子以明庶政",
        "image": "山下有火，贲。君子以明庶政，无敢折狱。",
        "judgement": "贲：亨。小利有攸往。",
        "yao_texts": [
            "初九：贲其趾，舍车而徒。",
            "六二：贲其须。",
            "九三：贲如濡如，永贞吉。",
            "六四：贲如皤如，白马翰如，匪寇婚媾。",
            "六五：贲于丘园，束帛戋戋，吝，终吉。",
            "上九：白贲，无咎。",
        ],
    },
    23: {
        "num": 23,
        "name": "剥",
        "symbol": "☶☷",
        "trigram_upper": "艮",
        "trigram_lower": "坤",
        "meaning": "山附于地，剥，上以厚下安宅",
        "image": "山附于地，剥。上以厚下安宅。",
        "judgement": "剥：不利有攸往。",
        "yao_texts": [
            "初六：剥床以足，蔑贞凶。",
            "六二：剥床以辨，蔑贞凶。",
            "六三：剥之，无咎。",
            "六四：剥床以肤，凶。",
            "六五：贯鱼，以宫人宠，无不利。",
            "上九：硕果不食，君子得舆，小人剥庐。",
        ],
    },
    24: {
        "num": 24,
        "name": "复",
        "symbol": "☷☳",
        "trigram_upper": "坤",
        "trigram_lower": "震",
        "meaning": "雷在地中，复，先王以至日闭关",
        "image": "雷在地中，复。先王以至日闭关，商旅不行，后不省方。",
        "judgement": "复：亨。出入无疾，朋来无咎。反复其道，七日来复，利有攸往。",
        "yao_texts": [
            "初九：不远复，无祇悔，元吉。",
            "六二：休复，吉。",
            "六三：频复，厉无咎。",
            "六四：中行独复。",
            "六五：敦复，无悔。",
            "上六：迷复，凶，有灾眚。用行师，终有大败，以其国君，凶；至于十年，不克征。",
        ],
    },
    25: {
        "num": 25,
        "name": "无妄",
        "symbol": "☰☳",
        "trigram_upper": "乾",
        "trigram_lower": "震",
        "meaning": "天下雷行，物与无妄",
        "image": "天下雷行，物与无妄。先王以茂对时，育万物。",
        "judgement": "无妄：元亨利贞。其匪正有眚，不利有攸往。",
        "yao_texts": [
            "初九：无妄，往吉。",
            "六二：不耕获，不菑畲，则利有攸往。",
            "六三：无妄之灾，或系之牛，行人之得，邑人之灾。",
            "九四：可贞，无咎。",
            "九五：无妄之疾，勿药有喜。",
            "上九：无妄，行有眚，无攸利。",
        ],
    },
    26: {
        "num": 26,
        "name": "大畜",
        "symbol": "☶☰",
        "trigram_upper": "艮",
        "trigram_lower": "乾",
        "meaning": "天在山中，大畜，君子以多识前言往行",
        "image": "天在山中，大畜。君子以多识前言往行，以畜其德。",
        "judgement": "大畜：利贞，不家食吉，利涉大川。",
        "yao_texts": [
            "初九：有厉，利已。",
            "九二：舆说辐。",
            "九三：良马逐，利艰贞。曰闲舆卫，利有攸往。",
            "六四：童牛之牿，元吉。",
            "六五：豶豕之牙，吉。",
            "上九：何天之衢，亨。",
        ],
    },
    27: {
        "num": 27,
        "name": "颐",
        "symbol": "☶☳",
        "trigram_upper": "艮",
        "trigram_lower": "震",
        "meaning": "山下有雷，颐，君子以慎言语，节饮食",
        "image": "山下有雷，颐。君子以慎言语，节饮食。",
        "judgement": "颐：贞吉。观颐，自求口实。",
        "yao_texts": [
            "初九：舍尔灵龟，观我朵颐，凶。",
            "六二：颠颐，拂经，于丘颐，征凶。",
            "六三：拂颐，贞凶，十年勿用，无攸利。",
            "六四：颠颐，吉。虎视眈眈，其欲逐逐，无咎。",
            "六五：拂经，居贞吉，不可涉大川。",
            "上九：由颐，厉吉，利涉大川。",
        ],
    },
    28: {
        "num": 28,
        "name": "大过",
        "symbol": "☱☴",
        "trigram_upper": "兑",
        "trigram_lower": "巽",
        "meaning": "泽灭木，大过，君子以独立不惧",
        "image": "泽灭木，大过。君子以独立不惧，遁世无闷。",
        "judgement": "大过：栋桡，利有攸往，亨。",
        "yao_texts": [
            "初六：藉用白茅，无咎。",
            "九二：枯杨生稊，老夫得其女妻，无不利。",
            "九三：栋桡，凶。",
            "九四：栋隆，吉。有它吝。",
            "九五：枯杨生华，老妇得其士夫，无咎无誉。",
            "上六：过涉灭顶，凶，无咎。",
        ],
    },
    29: {
        "num": 29,
        "name": "坎",
        "symbol": "☵☵",
        "trigram_upper": "坎",
        "trigram_lower": "坎",
        "meaning": "水洊至，习坎，君子以常德行，习教事",
        "image": "水洊至，习坎。君子以常德行，习教事。",
        "judgement": "习坎：有孚，维心亨，行有尚。",
        "yao_texts": [
            "初六：习坎，入于坎窞，凶。",
            "九二：坎有险，求小得。",
            "六三：来之坎坎，险且枕，入于坎窞，勿用。",
            "六四：樽酒簋贰，用缶，纳约自牖，终无咎。",
            "九五：坎不盈，祇既平，无咎。",
            "上六：系用徽纆，寘于丛棘，三岁不得，凶。",
        ],
    },
    30: {
        "num": 30,
        "name": "离",
        "symbol": "☲☲",
        "trigram_upper": "离",
        "trigram_lower": "离",
        "meaning": "明两作，离，大人以继明照于四方",
        "image": "明两作，离。大人以继明照于四方。",
        "judgement": "离：利贞，亨。畜牝牛，吉。",
        "yao_texts": [
            "初九：履错然，敬之无咎。",
            "六二：黄离，元吉。",
            "九三：日昃之离，不鼓缶而歌，则大耋之嗟，凶。",
            "九四：突如其来如，焚如，死如，弃如。",
            "六五：出涕沱若，戚嗟若，吉。",
            "上九：王用出征，有嘉折首，获匪其丑，无咎。",
        ],
    },
    31: {
        "num": 31,
        "name": "咸",
        "symbol": "☱☶",
        "trigram_upper": "兑",
        "trigram_lower": "艮",
        "meaning": "山上有泽，咸，君子以虚受人",
        "image": "山上有泽，咸。君子以虚受人。",
        "judgement": "咸：亨，利贞，取女吉。",
        "yao_texts": [
            "初六：咸其拇。",
            "六二：咸其腓，凶，居吉。",
            "九三：咸其股，执其随，往吝。",
            "九四：贞吉悔亡，憧憧往来，朋从尔思。",
            "九五：咸其脢，无悔。",
            "上六：咸其辅颊舌。",
        ],
    },
    32: {
        "num": 32,
        "name": "恒",
        "symbol": "☳☴",
        "trigram_upper": "震",
        "trigram_lower": "巽",
        "meaning": "雷风，恒，君子以立不易方",
        "image": "雷风，恒。君子以立不易方。",
        "judgement": "恒：亨，无咎，利贞，利有攸往。",
        "yao_texts": [
            "初六：浚恒，贞凶，无攸利。",
            "九二：悔亡。",
            "九三：不恒其德，或承之羞，贞吝。",
            "九四：田无禽。",
            "六五：恒其德，贞，妇人吉，夫子凶。",
            "上六：振恒，凶。",
        ],
    },
    33: {
        "num": 33,
        "name": "遁",
        "symbol": "☰☶",
        "trigram_upper": "乾",
        "trigram_lower": "艮",
        "meaning": "天下有山，遁，君子以远小人",
        "image": "天下有山，遁。君子以远小人，不恶而严。",
        "judgement": "遁：亨，小利贞。",
        "yao_texts": [
            "初六：遁尾，厉，勿用有攸往。",
            "六二：执之用黄牛之革，莫之胜说。",
            "九三：系遁，有疾厉，畜臣妾吉。",
            "九四：好遁，君子吉，小人否。",
            "九五：嘉遁，贞吉。",
            "上九：肥遁，无不利。",
        ],
    },
    34: {
        "num": 34,
        "name": "大壮",
        "symbol": "☳☰",
        "trigram_upper": "震",
        "trigram_lower": "乾",
        "meaning": "雷在天上，大壮，君子以非礼弗履",
        "image": "雷在天上，大壮。君子以非礼弗履。",
        "judgement": "大壮：利贞。",
        "yao_texts": [
            "初九：壮于趾，征凶，有孚。",
            "九二：贞吉。",
            "九三：小人用壮，君子用罔，贞厉。羝羊触藩，羸其角。",
            "九四：贞吉悔亡，藩决不羸，壮于大舆之輹。",
            "六五：丧羊于易，无悔。",
            "上六：羝羊触藩，不能退，不能遂，无攸利，艰则吉。",
        ],
    },
    35: {
        "num": 35,
        "name": "晋",
        "symbol": "☲☷",
        "trigram_upper": "离",
        "trigram_lower": "坤",
        "meaning": "明出地上，晋，君子以自昭明德",
        "image": "明出地上，晋。君子以自昭明德。",
        "judgement": "晋：康侯用锡马蕃庶，昼日三接。",
        "yao_texts": [
            "初六：晋如，摧如，贞吉。罔孚，裕无咎。",
            "六二：晋如，愁如，贞吉。受兹介福，于其王母。",
            "六三：众允，悔亡。",
            "九四：晋如鼫鼠，贞厉。",
            "六五：悔亡，失得勿恤，往吉无不利。",
            "上九：晋其角，维用伐邑，厉吉无咎，贞吝。",
        ],
    },
    36: {
        "num": 36,
        "name": "明夷",
        "symbol": "☷☲",
        "trigram_upper": "坤",
        "trigram_lower": "离",
        "meaning": "明入地中，明夷，君子以莅众，用晦而明",
        "image": "明入地中，明夷。君子以莅众，用晦而明。",
        "judgement": "明夷：利艰贞。",
        "yao_texts": [
            "初九：明夷于飞，垂其翼。君子于行，三日不食。有攸往，主人有言。",
            "六二：明夷，夷于左股，用拯马壮，吉。",
            "九三：明夷于南狩，得其大首，不可疾贞。",
            "六四：入于左腹，获明夷之心，于出门庭。",
            "六五：箕子之明夷，利贞。",
            "上六：不明晦，初登于天，后入于地。",
        ],
    },
    37: {
        "num": 37,
        "name": "家人",
        "symbol": "☴☲",
        "trigram_upper": "巽",
        "trigram_lower": "离",
        "meaning": "风自火出，家人，君子以言有物而行有恒",
        "image": "风自火出，家人。君子以言有物而行有恒。",
        "judgement": "家人：利女贞。",
        "yao_texts": [
            "初九：闲有家，悔亡。",
            "六二：无攸遂，在中馈，贞吉。",
            "九三：家人嗃嗃，悔厉吉；妇子嘻嘻，终吝。",
            "六四：富家，大吉。",
            "九五：王假有家，勿恤，吉。",
            "上九：有孚威如，终吉。",
        ],
    },
    38: {
        "num": 38,
        "name": "睽",
        "symbol": "☲☱",
        "trigram_upper": "离",
        "trigram_lower": "兑",
        "meaning": "上火下泽，睽，君子以同而异",
        "image": "上火下泽，睽。君子以同而异。",
        "judgement": "睽：小事吉。",
        "yao_texts": [
            "初九：悔亡，丧马勿逐，自复；见恶人无咎。",
            "九二：遇主于巷，无咎。",
            "六三：见舆曳，其牛掣，其人天且劓，无初有终。",
            "九四：睽孤，遇元夫，交孚，厉无咎。",
            "六五：悔亡，厥宗噬肤，往何咎。",
            "上九：睽孤，见豕负涂，载鬼一车，先张之弧，后说之弧，匪寇婚媾，往遇雨则吉。",
        ],
    },
    39: {
        "num": 39,
        "name": "蹇",
        "symbol": "☵☶",
        "trigram_upper": "坎",
        "trigram_lower": "艮",
        "meaning": "山上有水，蹇，君子以反身修德",
        "image": "山上有水，蹇。君子以反身修德。",
        "judgement": "蹇：利西南，不利东北。利见大人，贞吉。",
        "yao_texts": [
            "初六：往蹇，来誉。",
            "六二：王臣蹇蹇，匪躬之故。",
            "九三：往蹇来反。",
            "六四：往蹇来连。",
            "九五：大蹇朋来。",
            "上六：往蹇来硕，吉；利见大人。",
        ],
    },
    40: {
        "num": 40,
        "name": "解",
        "symbol": "☳☵",
        "trigram_upper": "震",
        "trigram_lower": "坎",
        "meaning": "雷雨作，解，君子以赦过宥罪",
        "image": "雷雨作，解。君子以赦过宥罪。",
        "judgement": "解：利西南，无所往，其来复吉。有攸往，夙吉。",
        "yao_texts": [
            "初六：无咎。",
            "九二：田获三狐，得黄矢，贞吉。",
            "六三：负且乘，致寇至，贞吝。",
            "九四：解而拇，朋至斯孚。",
            "六五：君子维有解，吉；有孚于小人。",
            "上六：公用射隼于高墉之上，获之，无不利。",
        ],
    },
    41: {
        "num": 41,
        "name": "损",
        "symbol": "☶☱",
        "trigram_upper": "艮",
        "trigram_lower": "兑",
        "meaning": "山下有泽，损，君子以惩忿窒欲",
        "image": "山下有泽，损。君子以惩忿窒欲。",
        "judgement": "损：有孚，元吉，无咎，可贞，利有攸往。曷之用，二簋可用享。",
        "yao_texts": [
            "初九：已事遄往，无咎，酌损之。",
            "九二：利贞，征凶，弗损益之。",
            "六三：三人行则损一人，一人行则得其友。",
            "六四：损其疾，使遄有喜，无咎。",
            "六五：或益之十朋之龟，弗克违，元吉。",
            "上九：弗损益之，无咎，贞吉，利有攸往，得臣无家。",
        ],
    },
    42: {
        "num": 42,
        "name": "益",
        "symbol": "☴☳",
        "trigram_upper": "巽",
        "trigram_lower": "震",
        "meaning": "风雷，益，君子以见善则迁，有过则改",
        "image": "风雷，益。君子以见善则迁，有过则改。",
        "judgement": "益：利有攸往，利涉大川。",
        "yao_texts": [
            "初九：利用为大作，元吉，无咎。",
            "六二：或益之十朋之龟，弗克违，永贞吉。王用享于帝，吉。",
            "六三：益之用凶事，无咎。有孚中行，告公用圭。",
            "六四：中行，告公从。利用为依迁国。",
            "九五：有孚惠心，勿问元吉。有孚惠我德。",
            "上九：莫益之，或击之，立心勿恒，凶。",
        ],
    },
    43: {
        "num": 43,
        "name": "夬",
        "symbol": "☱☰",
        "trigram_upper": "兑",
        "trigram_lower": "乾",
        "meaning": "泽上于天，夬，君子以施禄及下",
        "image": "泽上于天，夬。君子以施禄及下，居德则忌。",
        "judgement": "夬：扬于王庭，孚号，有厉，告自邑，不利即戎，利有攸往。",
        "yao_texts": [
            "初九：壮于前趾，往不胜为咎。",
            "九二：惕号，莫夜有戎，勿恤。",
            "九三：壮于頄，有凶。君子夬夬，独行遇雨，若濡有愠，无咎。",
            "九四：臀无肤，其行次且。牵羊悔亡，闻言不信。",
            "九五：苋陆夬夬，中行无咎。",
            "上六：无号，终有凶。",
        ],
    },
    44: {
        "num": 44,
        "name": "姤",
        "symbol": "☰☴",
        "trigram_upper": "乾",
        "trigram_lower": "巽",
        "meaning": "天下有风，姤，后以施命诰四方",
        "image": "天下有风，姤。后以施命诰四方。",
        "judgement": "姤：女壮，勿用取女。",
        "yao_texts": [
            "初六：系于金柅，贞吉，有攸往，见凶，羸豕孚蹢躅。",
            "九二：包有鱼，无咎，不利宾。",
            "九三：臀无肤，其行次且，厉，无大咎。",
            "九四：包无鱼，起凶。",
            "九五：以杞包瓜，含章，有陨自天。",
            "上九：姤其角，吝，无咎。",
        ],
    },
    45: {
        "num": 45,
        "name": "萃",
        "symbol": "☱☷",
        "trigram_upper": "兑",
        "trigram_lower": "坤",
        "meaning": "泽上于地，萃，君子以除戎器，戒不虞",
        "image": "泽上于地，萃。君子以除戎器，戒不虞。",
        "judgement": "萃：亨。王假有庙，利见大人，亨，利贞。用大牲吉，利有攸往。",
        "yao_texts": [
            "初六：有孚不终，乃乱乃萃，若号一握为笑，勿恤，往无咎。",
            "六二：引吉，无咎，孚乃利用禴。",
            "六三：萃如嗟如，无攸利，往无咎，小吝。",
            "九四：大吉，无咎。",
            "九五：萃有位，无咎。匪孚，元永贞，悔亡。",
            "上六：赍咨涕洟，无咎。",
        ],
    },
    46: {
        "num": 46,
        "name": "升",
        "symbol": "☷☴",
        "trigram_upper": "坤",
        "trigram_lower": "巽",
        "meaning": "地中生木，升，君子以顺德，积小以高大",
        "image": "地中生木，升。君子以顺德，积小以高大。",
        "judgement": "升：元亨，用见大人，勿恤，南征吉。",
        "yao_texts": [
            "初六：允升，大吉。",
            "九二：孚乃利用禴，无咎。",
            "九三：升虚邑。",
            "六四：王用亨于岐山，吉，无咎。",
            "六五：贞吉，升阶。",
            "上六：冥升，利于不息之贞。",
        ],
    },
    47: {
        "num": 47,
        "name": "困",
        "symbol": "☱☵",
        "trigram_upper": "兑",
        "trigram_lower": "坎",
        "meaning": "泽无水，困，君子以致命遂志",
        "image": "泽无水，困。君子以致命遂志。",
        "judgement": "困：亨，贞，大人吉，无咎，有言不信。",
        "yao_texts": [
            "初六：臀困于株木，入于幽谷，三岁不觌。",
            "九二：困于酒食，朱绂方来，利用享祀，征凶，无咎。",
            "六三：困于石，据于蒺藜，入于其宫，不见其妻，凶。",
            "九四：来徐徐，困于金车，吝，有终。",
            "九五：劓刖，困于赤绂，乃徐有说，利用祭祀。",
            "上六：困于葛藟，于臲卼，曰动悔。有悔，征吉。",
        ],
    },
    48: {
        "num": 48,
        "name": "井",
        "symbol": "☵☴",
        "trigram_upper": "坎",
        "trigram_lower": "巽",
        "meaning": "木上有水，井，君子以劳民劝相",
        "image": "木上有水，井。君子以劳民劝相。",
        "judgement": "井：改邑不改井，无丧无得，往来井井。汔至，亦未繘井，羸其瓶，凶。",
        "yao_texts": [
            "初六：井泥不食，旧井无禽。",
            "九二：井谷射鲋，瓮敝漏。",
            "九三：井渫不食，为我心恻，可用汲，王明，并受其福。",
            "六四：井甃，无咎。",
            "九五：井冽，寒泉食。",
            "上六：井收勿幕，有孚元吉。",
        ],
    },
    49: {
        "num": 49,
        "name": "革",
        "symbol": "☱☲",
        "trigram_upper": "兑",
        "trigram_lower": "离",
        "meaning": "泽中有火，革，君子以治历明时",
        "image": "泽中有火，革。君子以治历明时。",
        "judgement": "革：己日乃孚，元亨利贞，悔亡。",
        "yao_texts": [
            "初九：巩用黄牛之革。",
            "六二：己日乃革之，征吉，无咎。",
            "九三：征凶，贞厉，革言三就，有孚。",
            "九四：悔亡，有孚改命，吉。",
            "九五：大人虎变，未占有孚。",
            "上六：君子豹变，小人革面，征凶，居贞吉。",
        ],
    },
    50: {
        "num": 50,
        "name": "鼎",
        "symbol": "☲☴",
        "trigram_upper": "离",
        "trigram_lower": "巽",
        "meaning": "木上有火，鼎，君子以正位凝命",
        "image": "木上有火，鼎。君子以正位凝命。",
        "judgement": "鼎：元吉，亨。",
        "yao_texts": [
            "初六：鼎颠趾，利出否，得妾以其子，无咎。",
            "九二：鼎有实，我仇有疾，不我能即，吉。",
            "九三：鼎耳革，其行塞，雉膏不食，方雨亏悔，终吉。",
            "九四：鼎折足，覆公餗，其形渥，凶。",
            "六五：鼎黄耳金铉，利贞。",
            "上九：鼎玉铉，大吉，无不利。",
        ],
    },
    51: {
        "num": 51,
        "name": "震",
        "symbol": "☳☳",
        "trigram_upper": "震",
        "trigram_lower": "震",
        "meaning": "洊雷，震，君子以恐惧修省",
        "image": "洊雷，震。君子以恐惧修省。",
        "judgement": "震：亨。震来虩虩，笑言哑哑。震惊百里，不丧匕鬯。",
        "yao_texts": [
            "初九：震来虩虩，后笑言哑哑，吉。",
            "六二：震来厉，亿丧贝，跻于九陵，勿逐，七日得。",
            "六三：震苏苏，震行无眚。",
            "九四：震遂泥。",
            "六五：震往来厉，亿无丧，有事。",
            "上六：震索索，视矍矍，征凶。震不于其躬，于其邻，无咎。婚媾有言。",
        ],
    },
    52: {
        "num": 52,
        "name": "艮",
        "symbol": "☶☶",
        "trigram_upper": "艮",
        "trigram_lower": "艮",
        "meaning": "兼山，艮，君子以思不出其位",
        "image": "兼山，艮。君子以思不出其位。",
        "judgement": "艮：艮其背，不获其身，行其庭，不见其人，无咎。",
        "yao_texts": [
            "初六：艮其趾，无咎，利永贞。",
            "六二：艮其腓，不拯其随，其心不快。",
            "九三：艮其限，列其夤，厉薰心。",
            "六四：艮其身，无咎。",
            "六五：艮其辅，言有序，悔亡。",
            "上九：敦艮，吉。",
        ],
    },
    53: {
        "num": 53,
        "name": "渐",
        "symbol": "☴☶",
        "trigram_upper": "巽",
        "trigram_lower": "艮",
        "meaning": "山上有木，渐，君子以居贤德善俗",
        "image": "山上有木，渐。君子以居贤德善俗。",
        "judgement": "渐：女归吉，利贞。",
        "yao_texts": [
            "初六：鸿渐于干，小子厉，有言，无咎。",
            "六二：鸿渐于磐，饮食衎衎，吉。",
            "九三：鸿渐于陆，夫征不复，妇孕不育，凶；利御寇。",
            "六四：鸿渐于木，或得其桷，无咎。",
            "九五：鸿渐于陵，妇三岁不孕，终莫之胜，吉。",
            "上九：鸿渐于逵，其羽可用为仪，吉。",
        ],
    },
    54: {
        "num": 54,
        "name": "归妹",
        "symbol": "☳☱",
        "trigram_upper": "震",
        "trigram_lower": "兑",
        "meaning": "泽上有雷，归妹，君子以永终知敝",
        "image": "泽上有雷，归妹。君子以永终知敝。",
        "judgement": "归妹：征凶，无攸利。",
        "yao_texts": [
            "初九：归妹以娣，跛能履，征吉。",
            "九二：眇能视，利幽人之贞。",
            "六三：归妹以须，反归以娣。",
            "九四：归妹愆期，迟归有时。",
            "六五：帝乙归妹，其君之袂，不如其娣之袂良，月几望，吉。",
            "上六：女承筐无实，士刲羊无血，无攸利。",
        ],
    },
    55: {
        "num": 55,
        "name": "丰",
        "symbol": "☳☲",
        "trigram_upper": "震",
        "trigram_lower": "离",
        "meaning": "雷电皆至，丰，君子以折狱致刑",
        "image": "雷电皆至，丰。君子以折狱致刑。",
        "judgement": "丰：亨，王假之，勿忧，宜日中。",
        "yao_texts": [
            "初九：遇其配主，虽旬无咎，往有尚。",
            "六二：丰其蔀，日中见斗，往得疑疾，有孚发若，吉。",
            "九三：丰其沛，日中见沬，折其右肱，无咎。",
            "九四：丰其蔀，日中见斗，遇其夷主，吉。",
            "六五：来章，有庆誉，吉。",
            "上六：丰其屋，蔀其家，窥其户，阒其无人，三岁不觌，凶。",
        ],
    },
    56: {
        "num": 56,
        "name": "旅",
        "symbol": "☲☶",
        "trigram_upper": "离",
        "trigram_lower": "艮",
        "meaning": "山上有火，旅，君子以明慎用刑而不留狱",
        "image": "山上有火，旅。君子以明慎用刑而不留狱。",
        "judgement": "旅：小亨，旅贞吉。",
        "yao_texts": [
            "初六：旅琐琐，斯其所取灾。",
            "六二：旅即次，怀其资，得童仆贞。",
            "九三：旅焚其次，丧其童仆，贞厉。",
            "九四：旅于处，得其资斧，我心不快。",
            "六五：射雉一矢亡，终以誉命。",
            "上九：鸟焚其巢，旅人先笑后号啕。丧牛于易，凶。",
        ],
    },
    57: {
        "num": 57,
        "name": "巽",
        "symbol": "☴☴",
        "trigram_upper": "巽",
        "trigram_lower": "巽",
        "meaning": "随风，巽，君子以申命行事",
        "image": "随风，巽。君子以申命行事。",
        "judgement": "巽：小亨，利攸往，利见大人。",
        "yao_texts": [
            "初六：进退，利武人之贞。",
            "九二：巽在床下，用史巫纷若，吉无咎。",
            "九三：频巽，吝。",
            "六四：悔亡，田获三品。",
            "九五：贞吉悔亡，无不利。无初有终，先庚三日，后庚三日，吉。",
            "上九：巽在床下，丧其资斧，贞凶。",
        ],
    },
    58: {
        "num": 58,
        "name": "兑",
        "symbol": "☱☱",
        "trigram_upper": "兑",
        "trigram_lower": "兑",
        "meaning": "丽泽，兑，君子以朋友讲习",
        "image": "丽泽，兑。君子以朋友讲习。",
        "judgement": "兑：亨，利贞。",
        "yao_texts": [
            "初九：和兑，吉。",
            "九二：孚兑，吉，悔亡。",
            "六三：来兑，凶。",
            "九四：商兑，未宁，介疾有喜。",
            "九五：孚于剥，有厉。",
            "上六：引兑。",
        ],
    },
    59: {
        "num": 59,
        "name": "涣",
        "symbol": "☴☵",
        "trigram_upper": "巽",
        "trigram_lower": "坎",
        "meaning": "风行水上，涣，先王以享于帝立庙",
        "image": "风行水上，涣。先王以享于帝立庙。",
        "judgement": "涣：亨。王假有庙，利涉大川，利贞。",
        "yao_texts": [
            "初六：用拯马壮，吉。",
            "九二：涣奔其机，悔亡。",
            "六三：涣其躬，无悔。",
            "六四：涣其群，元吉。涣有丘，匪夷所思。",
            "九五：涣汗其大号，涣王居，无咎。",
            "上九：涣其血，去逖出，无咎。",
        ],
    },
    60: {
        "num": 60,
        "name": "节",
        "symbol": "☵☱",
        "trigram_upper": "坎",
        "trigram_lower": "兑",
        "meaning": "泽上有水，节，君子以制数度，议德行",
        "image": "泽上有水，节。君子以制数度，议德行。",
        "judgement": "节：亨。苦节不可贞。",
        "yao_texts": [
            "初九：不出户庭，无咎。",
            "九二：不出门庭，凶。",
            "六三：不节若，则嗟若，无咎。",
            "六四：安节，亨。",
            "九五：甘节，吉，往有尚。",
            "上六：苦节，贞凶，悔亡。",
        ],
    },
    61: {
        "num": 61,
        "name": "中孚",
        "symbol": "☴☱",
        "trigram_upper": "巽",
        "trigram_lower": "兑",
        "meaning": "泽上有风，中孚，君子以议狱缓死",
        "image": "泽上有风，中孚。君子以议狱缓死。",
        "judgement": "中孚：豚鱼吉，利涉大川，利贞。",
        "yao_texts": [
            "初九：虞吉，有它不燕。",
            "九二：鹤鸣在阴，其子和之。我有好爵，吾与尔靡之。",
            "六三：得敌，或鼓或罢，或泣或歌。",
            "六四：月几望，马匹亡，无咎。",
            "九五：有孚挛如，无咎。",
            "上九：翰音登于天，贞凶。",
        ],
    },
    62: {
        "num": 62,
        "name": "小过",
        "symbol": "☳☶",
        "trigram_upper": "震",
        "trigram_lower": "艮",
        "meaning": "山上有雷，小过，君子以行过乎恭",
        "image": "山上有雷，小过。君子以行过乎恭，丧过乎哀，用过乎俭。",
        "judgement": "小过：亨，利贞，可小事，不可大事。飞鸟遗之音，不宜上宜下，大吉。",
        "yao_texts": [
            "初六：飞鸟以凶。",
            "六二：过其祖，遇其妣；不及其君，遇其臣；无咎。",
            "九三：弗过防之，从或戕之，凶。",
            "九四：无咎，弗过遇之。往厉必戒，勿用永贞。",
            "六五：密云不雨，自我西郊，公弋取彼在穴。",
            "上六：弗遇过之，飞鸟离之，凶，是谓灾眚。",
        ],
    },
    63: {
        "num": 63,
        "name": "既济",
        "symbol": "☵☲",
        "trigram_upper": "坎",
        "trigram_lower": "离",
        "meaning": "水在火上，既济，君子以思患而预防之",
        "image": "水在火上，既济。君子以思患而预防之。",
        "judgement": "既济：亨，小利贞，初吉终乱。",
        "yao_texts": [
            "初九：曳其轮，濡其尾，无咎。",
            "六二：妇丧其茀，勿逐，七日得。",
            "九三：高宗伐鬼方，三年克之，小人勿用。",
            "六四：繻有衣袽，终日戒。",
            "九五：东邻杀牛，不如西邻之禴祭，实受其福。",
            "上六：濡其首，厉。",
        ],
    },
    64: {
        "num": 64,
        "name": "未济",
        "symbol": "☲☵",
        "trigram_upper": "离",
        "trigram_lower": "坎",
        "meaning": "火在水上，未济，君子以慎辨物居方",
        "image": "火在水上，未济。君子以慎辨物居方。",
        "judgement": "未济：亨，小狐汔济，濡其尾，无攸利。",
        "yao_texts": [
            "初六：濡其尾，吝。",
            "九二：曳其轮，贞吉。",
            "六三：未济，征凶，利涉大川。",
            "九四：贞吉，悔亡，震用伐鬼方，三年有赏于大国。",
            "六五：贞吉，无悔，君子之光，有孚，吉。",
            "上九：有孚于饮酒，无咎，濡其首，有孚失是。",
        ],
    },
}


def get_hexagram(num: int) -> Optional[dict]:
    """通过卦号（1-64）获取卦象数据。

    Args:
        num: 卦号，1-64

    Returns:
        卦象字典，若卦号无效返回 None
    """
    return HEXAGRAMS.get(num)


def get_hexagram_by_trigrams(upper: str, lower: str) -> Optional[dict]:
    """通过上下卦名获取卦象数据。

    Args:
        upper: 上卦名，如 "乾"
        lower: 下卦名，如 "坤"

    Returns:
        卦象字典，若卦名无效返回 None
    """
    upper_info = TRIGRAMS.get(upper)
    lower_info = TRIGRAMS.get(lower)
    if not upper_info or not lower_info:
        return None

    upper_bin = upper_info[0]
    lower_bin = lower_info[0]
    num = _KING_WEN_TABLE.get((upper_bin, lower_bin))
    if num is None:
        return None
    return HEXAGRAMS.get(num)


def trigram_binary_to_name(binary: int) -> Optional[str]:
    """将三位二进制值转换为卦名。

    Args:
        binary: 0-7 的整数

    Returns:
        卦名字符串，如 "乾"；无效值返回 None
    """
    return _BINARY_TO_TRIGRAM.get(binary)


def yao_to_trigram_binaries(yao_sequence: str) -> tuple[int, int]:
    """将六爻序列转换为上下卦的二进制值。

    爻序列中每个数字的含义：
        6 = 老阴（变爻，本卦为阴）
        7 = 少阳（不变，阳）
        8 = 少阴（不变，阴）
        9 = 老阳（变爻，本卦为阳）

    本卦取法：6->阴(0), 7->阳(1), 8->阴(0), 9->阳(1)

    Args:
        yao_sequence: 6位字符串，如 "679867"，从初爻到上爻

    Returns:
        (上卦二进制, 下卦二进制) 元组

    Raises:
        ValueError: 若序列格式不正确
    """
    if len(yao_sequence) != 6 or not all(c in "6789" for c in yao_sequence):
        raise ValueError(f"无效的爻序列: {yao_sequence}，必须是6位，每位为6/7/8/9")

    # 将爻值转换为阴阳：6,8->0(阴), 7,9->1(阳)
    bits = []
    for c in yao_sequence:
        val = int(c)
        bits.append(1 if val % 2 == 1 else 0)  # 奇数为阳，偶数为阴

    # 下卦：初爻(0)、二爻(1)、三爻(2)
    lower = (bits[2] << 2) | (bits[1] << 1) | bits[0]
    # 上卦：四爻(3)、五爻(4)、上爻(5)
    upper = (bits[5] << 2) | (bits[4] << 1) | bits[3]

    return upper, lower


def yao_sequence_to_hexagram_num(yao_sequence: str) -> int:
    """将六爻序列转换为卦号（1-64）。

    Args:
        yao_sequence: 6位字符串

    Returns:
        卦号 1-64

    Raises:
        ValueError: 若序列无效或无法映射到卦号
    """
    upper, lower = yao_to_trigram_binaries(yao_sequence)
    num = _KING_WEN_TABLE.get((upper, lower))
    if num is None:
        raise ValueError(f"无法将 (上卦={upper:03b}, 下卦={lower:03b}) 映射到卦号")
    return num


def get_changing_yao(yao_sequence: str) -> list[int]:
    """获取变爻位置列表。

    变爻：6(老阴) 和 9(老阳) 为变爻。

    Args:
        yao_sequence: 6位字符串

    Returns:
        变爻位置列表（1-based），如 [1, 3] 表示第1爻和第3爻为变爻
    """
    changing = []
    for i, c in enumerate(yao_sequence):
        if c in ("6", "9"):
            changing.append(i + 1)  # 1-based
    return changing


def get_changed_yao_sequence(yao_sequence: str) -> Optional[str]:
    """获取变卦后的爻序列。

    变爻规则：6(老阴)->7(少阳), 9(老阳)->8(少阴)
    不变爻：7->7, 8->8

    Args:
        yao_sequence: 6位原始爻序列

    Returns:
        变卦后的爻序列；若无变爻则返回 None
    """
    changing = get_changing_yao(yao_sequence)
    if not changing:
        return None

    change_map = {"6": "7", "7": "7", "8": "8", "9": "8"}
    return "".join(change_map[c] for c in yao_sequence)
